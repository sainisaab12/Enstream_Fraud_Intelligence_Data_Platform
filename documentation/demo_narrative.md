# EnStream Fraud Intelligence Platform - 30-Minute Demonstration Narrative (Architecture Evolution Edition)

This document provides a detailed, step-by-step presentation outline and narrative for a **30-minute live demonstration** of the **EnStream Fraud Intelligence Platform** prototype. The demo is structured directly around the **5-Phase Transition Roadmap** and leverages our interactive **Architecture Evolution** console.

---

## 30-Minute Demo Segment Breakdown

| Roadmap Phase | Demo Segment | Key Prototype Clicks | Focus Area | Duration |
| :--- | :--- | :--- | :--- | :--- |
| **Setup** | Opening Screen & Context | `Technical Working` -> `Phase Roadmap` | Project context, objectives, legacy setup | 3 Mins |
| **Phase 1-2** | Batch Baseline | `Architecture Evolution` [Phase 1-2] -> `Medallion Spec` -> `Ops Center` | Legacy MySQL scope, Spark batch promotions, MWAA scheduler | 5 Mins |
| **Phase 2** | Batch Automated | `Architecture Evolution` -> `DQ Monitor` -> `ML Registry` | SLA Rules Ledger, automated self-healing, Drift monitor (PSI) | 5 Mins |
| **Phase 3** | Interim Real-Time | `Architecture Evolution` [Phase 3] -> `Partner API Sandbox` | Kinesis + Lambda serverless ingestion, 13 REST APIs, JWS/JWE wraps | 5 Mins |
| **Phase 4** | Source-Direct Real-Time | `Architecture Evolution` [Phase 4] -> `Cache Monitor` -> `Query Investigator` | Flink CEP, RocksDB state, sub-10ms Redis cache hits, BFS Graph walk | 6 Mins |
| **Phase 5** | Continuous Everything | `Architecture Evolution` [Phase 5] -> `Exchange Hub` -> `Executive Desk` | bad actor exchange, GDPR PII masking, retraining loops | 4 Mins |
| **Summary** | Demo Conclusion & Q&A | `Technical Working` -> `Vendor Brief` | Competitive positioning vs Subex/Fraudzap, open-table advantages | 2 Mins |
...
- Click on the **Technical Working** tab in the left sidebar.
- Select the **Phase Roadmap** subtab.
...
## Segment 2: Phase 1 & 2 - Batch Baseline (5 Mins)

### Spoken Word Target: ~680 words
### Visual Clicks & Console Actions:
1. Under **Technical Working**, click on the **Architecture Evolution** subtab.
2. Select **Phase 1-2 (Batch)** on the selector.
3. Point to the **Phase 1 & 2 Batch Baseline Architecture** diagram on the right, and the **Key Technologies** and **Acceptance Gate Criteria** on the left.
4. Navigate to the **Medallion Spec** tab in the sidebar. Scroll to the bottom to show the conformed MySQL tables scope.
5. Navigate to the **Ops Center** tab to show the scrolling CLI logs of the Spark batch promotion.
...
### Visual Clicks & Console Actions:
1. Navigate back to **Technical Working** -> **Architecture Evolution** and review the Phase 2 automation details.
2. Navigate to the **DQ Monitor** tab in the sidebar.
3. Review the **SLA Rules Audit Ledger** table, showing checks like `CHECK_MSISDN_FORMAT`, `CHECK_IMEI_LENGTH`, and `CHECK_FRESHNESS_SLA` showing a green `PASS` state.
...
5. Click on the **ML Registry** tab in the sidebar. Point out the Drift Monitoring panel showing the Population Stability Index (PSI) graph.
...
### Spoken Word Target: ~700 words
### Visual Clicks & Console Actions:
1. Navigate back to **Technical Working** -> **Architecture Evolution**.
2. Select **Phase 3 (Serverless Stream)** on the selector.
3. Point to the **Phase 3 Serverless Real-Time Ingest Architecture** diagram on the right, showing the Kinesis -> Lambda serverless stream.
4. Select the **Partner API Sandbox** subtab at the top.
5. Show the **Select Guide Rest API** list. Select `A1: Basic Identity Verification (IDV) API` and `D3M: Recent Service Changes API (Multi)`.
6. Check the box **"Enable JOSE JWS/JWE Protection (RFC7515/7516)"** and click **"Send Rest API Request"**. Point out the terminal log displaying JWS signature headers and encrypted envelopes.
...
### Visual Clicks & Console Actions:
1. Navigate back to **Technical Working** -> **Architecture Evolution**.
2. Select **Phase 4 (Stateful Stream)** on the selector.
3. Point to the **Phase 4 Stateful Enterprise Ingest Architecture** diagram on the right, showing the Rogers/Bell/Telus -> Apache Flink CEP pipeline.
4. Select the **Partner API Sandbox** subtab at the top, and scroll down to the **Low-Latency Cache & Performance Simulation Monitor**.
5. Select cache options to show latency updates:
...
6. Navigate to the **Event Ingress** tab in the sidebar. Show how manual event triggers (e.g. sending a Rogers SIM Swap event for `14165559001`) launch visual telemetry pulse animations.
7. Navigate to the **Query Investigator** tab in the sidebar. Select `14165559001` or `14165559013`. Explain:
...
### Visual Clicks & Console Actions:
1. Navigate back to **Technical Working** -> **Architecture Evolution**.
2. Select **Phase 5 (Northstar)** on the selector.
3. Point to the **Phase 5 Northstar Systems Architecture** diagram on the right, showing the full cloud topology with retraining pipeline loops and Bad Actor Exchange Hub.
4. Navigate to the **Exchange Hub** tab in the sidebar.
5. Go to the **Real-Time Cross-Sector Lookup** card. Perform a lookup:
   - Note the porting checks, TransUnion SPID verification, and PII match score.
   - Change the role selection at the top from `Fraud Analyst` to `Public User` and observe the PII name and phone number fields instantly mask/redact.
6. Point out the **Exchange Success Metrics** charts (Prevention Ratio: ~74%, Match Rate: ~88%).
7. Navigate to the **Executive Desk** tab in the sidebar. Review the consolidated high-level metrics, fraud alerts, and the interactive map showing the bad actor nodes.
